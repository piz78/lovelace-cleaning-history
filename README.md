...
| `tap_action`        | action | no       | open image in modal         | Action on tap. By default, opens an in-card dialog styled like HA's native more-info dialog. Any standard HA action (`navigate`, `url`, `call-service`, `more-info`, `none`, etc.) overrides this. |
| `hold_action`       | action | no       | none                         | Action on hold (long-press).                                                                              |
| `double_tap_action` | action | no       | none                         | Action on double-tap.                                                                                     |

The card lists every key of the chosen attribute in a dropdown (newest first, matching the entity's own ordering) and displays the corresponding image below it. By default, clicking the image opens an in-card dialog styled like HA's native more-info dialog, showing the exact historical snapshot selected in the dropdown — unlike the entity's actual more-info dialog, which only ever shows its current/live picture. Setting an explicit `tap_action` (e.g. `more-info`) overrides this and will show the live picture instead of the selected snapshot.
